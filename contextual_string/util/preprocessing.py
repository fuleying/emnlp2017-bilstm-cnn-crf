...
def perpareDataset(datasets, embeddingsClass, padOneTokenSentence=True,
    ...
    outputPath = 'data/Lambert/pkl/' + pklName + '.pkl'
    ...
    for datasetName, dataset in datasets.items():
        ...
        pklObjects['data'][datasetName] = createPklFiles(paths, mappings, datasetColumns,
                        commentSymbol, padOneTokenSentence, valTransformations)
...
def addEmbeddings(sentences, embeddingsFct, padOneTokenSentence=True):
    ...
    for embeddingName in word_embeddings.keys():  # ['tokens', 'constring']
        ...
        for sentenceIdx in range(len(sentences)):
            ...
            if padOneTokenSentence and len(sentence[embeddingName+'_embeddings']) == 1:
                ...
                sentence[embeddingName + '_embeddings'] = np.append(
                    sentence[embeddingName+'_embeddings'], zeros, axis=0)
...
def createMatrices(sentences, mappings, padOneTokenSentence):
    ...
    mappings: {'tokens': {}, 'characters': {}, 'casing': {}, 'NER_BIO': {},
               'NER_class': {}, 'NER_IOB': {}, 'NER_IOBES': {}}
    Returns:
    data: {'tokens': [], 'characters': [], 'casing': [], 'POS':[],
           'NER_BIO': [], 'NER_class': [], 'NER_IOB': [], 'NER_IOBES': []}
          `tokens` is a string list, others are all idx list.
...
def createPklFiles(datasetFiles, mappings, cols, commentSymbol, padOneTokenSentence, valTransformation):
