...
class ELMoWordEmbeddings:
    def __init__(self, embeddings_path, elmo_options_file=DEFAULT_OPTIONS_FILE,
                 elmo_weight_file=DEFAULT_WEIGHT_FILE, elmo_mode='weighted_average',
                 elmo_cuda_device=-1):
    ...
    def getElmoEmbedding(self, sentences):
        ...
        for sentence in sentences:
            ...
            if len(self.cache) > 0 and cache_key in self.cache:
                ...
                # the index in elmo_embeddings
                non_cached_sentences_indices.append(len(elmo_embeddings))
                # the Placeholder in elmo_embeddings
                elmo_embeddings.append(None)  # len(elmo_embeddings) + 1
