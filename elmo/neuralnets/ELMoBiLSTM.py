"""
A bidirectional LSTM with optional CRF and character-based presentation
for NLP sequence tagging used for multi-task learning.
...
class ELMoBiLSTM:
    def __init__(self, embeddingsLookup, params=None):
        ...
        if embConfig['embeddings_path'] is not None \
            and 'tokens_embeddings' not in self.params['featureNames']:
            self.params['featureNames'].append('tokens_embeddings')

        if embConfig['elmo_mode'] is not None \
            and 'elmo_embeddings' not in self.params['featureNames']:
    ...
    def buildModel(self):
        ...
        for featureName in self.params['featureNames']:
            ...
            if featureName.endswith('_embeddings'):
                ...
                # Tokens Embeddings (100,) or average_elmo_embeddings (1024,)
                if len(sampleEmbedding.shape)==1:
                    # [batch_size, max_seq_len, 100]
                    tokens_input = Input(shape=(None, sampleEmbedding.shape[0]),
                                         dtype='float32',
                                         name=featureName + '_input')
                    inputNodes.append(tokens_input)
                    mergeInputLayers.append(tokens_input)
                # weighted average ELMo Embeddings (3, 1024)
                elif len(sampleEmbedding.shape)==2:
                    # [batch_size, max_seq_len, 3, 1024]
                    elmo_input = Input(shape=(None, sampleEmbedding.shape[0],
                                              sampleEmbedding.shape[1]),
                                       dtype='float32',
                                       name=featureName + '_input')
                    # [batch_size, max_seq_len, 1024]
                    elmo_output = TimeDistributed(WeightedAverage(),
                        name=featureName + '_weighted_average')(elmo_input)
    ...
    def minibatch_iterate_dataset(self, modelNames = None):
        """ Create based on sentence length mini-batches with approx. the same size.
        Sentences and mini-batch chunks are shuffled and used to the train the model """
    ...
    def computeF1(self, modelName, sentences):
        ...
        pre, rec, f1 = BIOF1Validation.compute_f1(
            predLabels, correctLabels, idx2Label, 'O', encodingScheme)
        ...
        for sentenceIdx in range(len(sentences)):
            for tokenIdx in range(len(sentences[sentenceIdx]['characters'])):
                ...
                if len(token) < maxCharLen:   # pad token 0 in the end
                    sentences[sentenceIdx]['characters'][tokenIdx] = np.pad(
                        token, (0,maxCharLen-len(token)), 'constant')
    ...
    def saveModel(self, modelName, epoch, dev_score, test_score):
        ...
        filename = "elmo_" + modelName + "_latest.h5"
    ...
    def loadModel(modelPath):
        ...
        embLookup = ELMoWordEmbeddings(embeddings_path, elmo_options_file,
