...
class CRF(Layer):
    ...
    def call(self, X, mask=None):
        ...
        if self.learn_mode == 'join':
            # train_output: [batch_size, seq_len, units]
    ...
    def step(self, input_energy_t, states, return_logZ=True):
        ...
        # input_energy_t: [batch_size, units]
        # prev_target_val: [batch_size, units]
        ...
        if return_logZ:
            ...
            # chain_energy: [1, units, units]
            # energy: [batch_size, units, units]
            energy = chain_energy + K.expand_dims(input_energy_t + prev_target_val, 2)
            # min_energy: [batch_size, units]
            min_energy = K.min(energy, 1)
            # argmin_table: [batch_size, units]
    ...
    def recursion(self, input_energy, mask=None, go_backwards=False, return_sequences=True, return_logZ=True, input_length=None):
        ...
        # chain_energy: (1, units, units): 1st units is for t-1, 2nd units for t
        chain_energy = K.expand_dims(chain_energy, 0)
        # input_energy: [batch_size, seq_len, units]
        # prev_target_val: (batch_size, units), dtype=float32
    ...
    def viterbi_decoding(self, X, mask=None):
        ...
        # argmin_tables: [batch_size, seq_len, units]
