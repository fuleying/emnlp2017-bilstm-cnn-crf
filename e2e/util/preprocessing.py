...
def perpareDataset(embeddingsPath, datasets, frequencyThresholdUnknownTokens=50,
    ...
    # Arguments:
        ...
        valTransformations: func param, Column specific value transformations
        padOneTokenSentence: True to pad `PADDING_TOKEN` if sentence only has one token (needed for CRF classifier)
    """
    embeddingsName = os.path.splitext(os.path.basename(embeddingsPath))[0] # "komninos_english_embeddings"
    pklName = "_".join(sorted(datasets.keys()) + [embeddingsName])
    outputPath = 'data/Lambert/pkl/' + pklName + '.pkl' # 'pkl/unidep_pos_komninos_english_embeddings.pkl'
    ...
    for datasetName, dataset in datasets.items():
        ...
        trainData = 'data/Lambert/%s/train.txt' % datasetName
        devData = 'data/Lambert/%s/dev.txt' % datasetName
        testData = 'data/Lambert/%s/test.txt' % datasetName
        paths = [trainData, devData, testData]

        logging.info(":: Transform "+datasetName+" dataset ::")
        pklObjects['data'][datasetName] = createPklFiles(paths, mappings, datasetColumns,
                        commentSymbol, valTransformations, padOneTokenSentence)
...
def readEmbeddings(embeddingsPath, datasetFiles, frequencyThresholdUnknownTokens,
    ...
    :return:
        1. filter word embeddings that not in train/val/test dataset;
        2. add word embeddings for PADDING_TOKEN, UNKNOWN_TOKEN
        3. add word embeddings for words not in embedding dataset but most frequency
    """
    ...
    if reducePretrainedEmbeddings:
        ...
        for datasetName in datasetFiles:
            dataColumnsIdx = {y: x for x, y in datasetFiles[datasetName]['columns'].items()}
            tokenIdx = dataColumnsIdx['tokens']
            datasetPath = 'data/Lambert/%s/' % datasetName
    ...
    def createFD(filename, tokenIndex, fd, word2Idx):
        for line in open(filename):
            ...
            # line = "1	What	what	PRON	WP	PronType=Int	0	root	_	_"
            splits = line.strip().split()
...
def addCharInformation(sentences):
    ...
    [what, is, that] ---> [[w, h, a, t], [i, s], [t, h, a, t]]
    """
...
def createMatrices(sentences, mappings, padOneTokenSentence):
    """Convert sentences from string list to idx list.
    Param:
    sentences: [sentence1, sentence2, sentence3, ...]
    sentence: {'tokens': [], 'characters': [], 'casing': [], 'NER_BIO': [],
               'NER_class': [], 'NER_IOB': [], 'NER_IOBES': []}
    mappings: {'tokens': {}, 'characters': {}, 'casing': {}, 'NER_BIO': {},
               'NER_class': {}, 'NER_IOB': {}, 'NER_IOBES': {}}
    Returns:
    data: {'raw_tokens':[], 'tokens': [], 'characters': [], 'casing': [],
           'NER_BIO': [], 'NER_class': [], 'NER_IOB': [], 'NER_IOBES': []}
          `raw_tokens` is a string list, others are all idx list.
          If sentence only has one token, add 'PADDING_TOKEN'.
    """
    data = []
    numTokens = 0
    numUnknownTokens = 0
...
def extendMappings(mappings, sentences):
    '''
    Add mappings for labels. sunch as:
        {'POS': [],
         'NER_BIO': [],
         'NER_class': [],
         'NER_IOB': [],
    ...
    for sentence in sentences:
        for name in sentenceKeys:
            if name not in mappings:
                mappings[name] = {'O':0} #'O' is also used for padding and 'O'
